...
function BoothApp() {
  ...
  if (peekScores) {
    ...
    screen = <SetupScreen roomCode={room} players={players} tracks={tracks} round={round} draft={draft} setDraft={setDraft} voting={Game.votingConfig(ps)} onVoting={(v) => Game.setVoting(room, v)} onBack={() => Game.setStatus(room, 'lobby')} onStart={() => Game.startRound(room, draft, players, tracks)} onUpload={upload} onTag={(id, tag) => Game.setTrackTag(id, tag)} onDeleteTrack={(id, title) => { if (window.confirm(`Delete “${title}” from the library? This removes the audio file for everyone and can’t be undone.`)) Game.deleteTrack(id); }} />;
  } else if (status === 'live') {
